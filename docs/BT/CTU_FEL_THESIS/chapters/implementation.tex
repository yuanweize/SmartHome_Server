...
\midrule
\multirow{7}{*}{MPU6050 (GY-521)} 
  & VCC & 3V3 & Power (2.375--3.46\,V) & VDD range per datasheet~\cite{mpu6050_datasheet_2013} \\
  & GND & GND & Ground & --- \\
  & SCL & GPIO4 & I2C Clock & Shared with BMP180 \\
  & SDA & GPIO5 & I2C Data & Shared with BMP180 \\
  & XDA/XCL & --- & Aux I2C & Unused; for external magnetometer \\
  & AD0 & --- & Address select & Unconnected = 0x68 \\
  & INT & --- & Interrupt & Unused; polling mode used \\
\midrule
\multirow{4}{*}{BMP180 (GY-68)} 
  & VIN & 3V3 & Power (1.8--3.6\,V) & Onboard regulator~\cite{bmp180_datasheet_2013} \\
  & GND & GND & Ground & --- \\
  & SCL & GPIO4 & I2C Clock & Shared with MPU6050 \\
  & SDA & GPIO5 & I2C Data & Shared with MPU6050 \\
\midrule
\multirow{4}{*}{KY-037 Sound} 
  & VCC & 3V3 & Power (3.3--5\,V) & --- \\
  & GND & GND & Ground & --- \\
  & AO & GPIO6 & Analog output & Continuous sound level \\
  & DO & --- & Digital output & Unused; AO preferred for granularity \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\midrule
\multirow{6}{*}{TCS34725} 
  & VIN & 3V3 & Power (2.7--3.6\,V) & VDD range per datasheet~\cite{tcs34725_datasheet_2012} \\
  & GND & GND & Ground & --- \\
...
\midrule
\multirow{4}{*}{MQ-2 Gas} 
  & VCC & VIN (5V) & Heater power & \textbf{$V_H = 5.0\,\text{V} \pm 0.2\,\text{V}$}~\cite{mq2_datasheet_2014} \\
  & GND & GND & Ground & --- \\
  & AO & GPIO34 & Analog output & $R_s$ varies with gas concentration \\
  & DO & --- & Digital output & Unused; AO preferred for resolution \\
\midrule
\multirow{3}{*}{SR602 PIR} 
  & VCC & 3V3 & Power (3.3--5\,V) & Ultra-low power~\cite{sr602_datasheet_2018} \\
  & GND & GND & Ground & --- \\
  & OUT & GPIO23 & Digital output & High on motion detection \\
\midrule
\multirow{4}{*}{RGB LED (Common Cathode)} 
  & R & GPIO26 & Red anode & LEDC PWM (8-bit) \\
  & G & GPIO27 & Green anode & LEDC PWM (8-bit) \\
  & B & GPIO14 & Blue anode & LEDC PWM (8-bit) \\
  & K & GND & Common cathode & --- \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\IfFileExists{images/py_chartflow.png}{%
    \includegraphics[width=0.85\linewidth]{images/py_chartflow.png}%
}{%
...
\begin{figure}[htbp]
    ...
    \IfFileExists{images/ha_dashboard.png}{%
        \includegraphics[width=1\linewidth]{images/ha_dashboard.png}%
    }{%
...
\begin{figure}[H]
    ...
    \IfFileExists{images/influxdb.png}{%
        \includegraphics[width=0.95\linewidth]{images/influxdb.png}%
    }{%
        \fbox{\parbox{0.95\linewidth}{\centering
...
\begin{figure}[H]
    ...
    \IfFileExists{images/node-red.png}{%
        \includegraphics[width=0.95\linewidth]{images/node-red.png}%
    }{%
        \fbox{\parbox{0.95\linewidth}{\centering
...
\begin{figure}[H]
    ...
    \IfFileExists{images/red.png}{%
        \includegraphics[width=0.85\linewidth]{images/red.png}%
    }{%
        \fbox{\parbox{0.95\linewidth}{\centering
...
Home Assistant provides built-in backup functionality supporting both local and cloud storage destinations. The deployed system implements a two-tier backup strategy: local backups are created every three days with 14 retained copies (covering approximately six weeks of recovery points), while cloud backups to Home Assistant Cloud are performed daily with 30-day retention. This configuration ensures rapid recovery from local storage for recent issues while maintaining off-site protection against hardware failures or local disasters.
